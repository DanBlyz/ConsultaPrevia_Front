...
export class ViajeFacade {
  ...
  get Filtro$(): Observable<ViajeFilter> {
    return this.store.select(filtroViajeSelector);
  }
  ...
  constructor(
    ...
    this.store.select(filtroViajeSelector).subscribe((filtro) => {
      if (filtro != null) {
        this.filtro = filtro;
      }
    });
    this.store.select(paginadoViajeSelector).subscribe((paginado) => {
      if (paginado != null) {
  ...
  establecerFiltro(filtro: ViajeFilter): void {
    this.store.dispatch(
      CorrespondenciaAcciones.establecerFiltroViaje({
        filtro: { ...filtro }
      })
    );
  }
  ...
  buscar(
    ...
    return new Promise((resolve) => {
      this.viajeService
        ...
        .subscribe((respuesta: RespuestaLista<Viaje>) => {
          this.store.dispatch(
            CorrespondenciaAcciones.establecerListaViaje({
  ...
  obtenerPorId(id: number): Promise<RespuestaObjeto<Viaje>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.viajeService
        .obtenerPorId(id)
        .subscribe((respuesta: RespuestaObjeto<Viaje>) => {
          this.store.dispatch(
            CorrespondenciaAcciones.establecerViaje({
  ...
  guardar(objeto: Viaje): Promise<RespuestaObjeto<Viaje>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
  ...
  modificar(id: number, objeto: Viaje): Promise<RespuestaObjeto<Viaje>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
  ...
  eliminar(id: number): Promise<Respuesta> {
    ...
    return new Promise((resolve) => {
      this.viajeService.eliminar(id).subscribe((respuesta: Respuesta) => {
        if (respuesta.tipoRespuesta === 'Exito') {
          if (this.paginado) {
            this.buscar(
              this.filtro,
