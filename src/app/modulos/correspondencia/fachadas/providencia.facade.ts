...
export class ProvidenciaFacade {
  ...
  get Filtro$(): Observable<ProvidenciaFilter> {
    return this.store.select(filtroProvidenciaSelector);
  }
  ...
  constructor(
    ...
    this.store.select(filtroProvidenciaSelector).subscribe((filtro) => {
      if (filtro != null) {
        this.filtro = filtro;
      }
    });
    this.store.select(paginadoProvidenciaSelector).subscribe((paginado) => {
      if (paginado != null) {
  ...
  establecerFiltro(filtro: ProvidenciaFilter): void {
    this.store.dispatch(
      CorrespondenciaAcciones.establecerFiltroProvidencia({
        filtro: { ...filtro }
      })
    );
  }
  ...
  buscar(
    objeto: ProvidenciaFilter,
    pagina: number,
    cantidad: number
  ): Promise<RespuestaLista<Providencia>> {
    this.store.dispatch(
    ...
    return new Promise((resolve) => {
      this.providenciaService
        ...
        .subscribe((respuesta: RespuestaLista<Providencia>) => {
          this.store.dispatch(
  ...
  obtenerPorId(id: number): Promise<RespuestaObjeto<Providencia>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.providenciaService
        .obtenerPorId(id)
        .subscribe((respuesta: RespuestaObjeto<Providencia>) => {
          this.store.dispatch(
            CorrespondenciaAcciones.establecerProvidencia({
  ...
  guardar(objeto: Providencia): Promise<RespuestaObjeto<Providencia>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
  ...
  modificar(id: number, objeto: Providencia): Promise<RespuestaObjeto<Providencia>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
  ...
  eliminar(id: number): Promise<Respuesta> {
    ...
    return new Promise((resolve) => {
      this.providenciaService.eliminar(id).subscribe((respuesta: Respuesta) => {
        if (respuesta.tipoRespuesta === 'Exito') {
          if (this.paginado) {
            this.buscar(
              this.filtro,
