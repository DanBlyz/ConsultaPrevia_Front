import { Injectable } from '@angular/core';
import { Observable } from 'rxjs';
import { Store } from '@ngrx/store';

import { AppState } from '../../../app.state';
import {
  Codificador,
  Paginado,
  Respuesta,
  RespuestaLista,
  RespuestaObjeto
} from '../../../comun/modelos';
import { ComunState } from '../../../comun/estados';
import * as ComunAcciones from '../../../comun/estados';

import { CorrespondenciaState } from '../estados';
import * as CorrespondenciaAcciones from '../estados/acciones';
import {
  filtroSujetoIdentificadoSelector,
  paginadoSujetoIdentificadoSelector
} from '../estados/selectores';
import { SujetoIdentificado } from '../modelos';
import { SujetoIdentificadoFilter } from '../modelos/filtros';
import { SujetoIdentificadoService } from '../servicios';

import { ClasificacionService } from '../servicios';

@Injectable({
  providedIn: 'root'
})
export class SujetoIdentificadoFacade {
  filtro: SujetoIdentificadoFilter = new SujetoIdentificadoFilter();
  paginado: Paginado;

  get ComunState$(): Observable<ComunState> {
    return this.store.select('comun');
  }

  get CorrespondenciaState$(): Observable<CorrespondenciaState> {
    return this.store.select('correspondencia');
  }

  get Filtro$(): Observable<SujetoIdentificadoFilter> {
    return this.store.select(filtroSujetoIdentificadoSelector);
  }

  constructor(
    private store: Store<AppState>,
    private sujetoIdentificadoService: SujetoIdentificadoService,
    private clasificacionService: ClasificacionService
  ) {
    this.store.select(filtroSujetoIdentificadoSelector).subscribe((filtro) => {
      if (filtro != null) {
        this.filtro = filtro;
      }
    });
    this.store.select(paginadoSujetoIdentificadoSelector).subscribe((paginado) => {
      if (paginado != null) {
        this.paginado = paginado;
      }
    });
  }

  establecerFiltro(filtro: SujetoIdentificadoFilter): void {
    this.store.dispatch(
      CorrespondenciaAcciones.establecerFiltroSujetoIdentificado({
        filtro: { ...filtro }
      })
    );
  }

  buscar(
    objeto: SujetoIdentificadoFilter,
    pagina: number,
    cantidad: number
  ): Promise<RespuestaLista<SujetoIdentificado>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService
        .buscar(objeto, pagina, cantidad)
        .subscribe((respuesta: RespuestaLista<SujetoIdentificado>) => {
          this.store.dispatch(
            CorrespondenciaAcciones.establecerListaSujetoIdentificado({
              lista: [...respuesta.lista],
              paginado: { ...respuesta.paginado }
            })
          );
          this.store.dispatch(
            ComunAcciones.establecerProcesando({ procesando: false })
          );
          resolve(respuesta);
        });
    });
  }

  obtenerPorId(id: number): Promise<RespuestaObjeto<SujetoIdentificado>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService
        .obtenerPorId(id)
        .subscribe((respuesta: RespuestaObjeto<SujetoIdentificado>) => {
          this.store.dispatch(
            CorrespondenciaAcciones.establecerSujetoIdentificado({
              objeto: { ...respuesta.objeto }
            })
          );
          this.store.dispatch(
            ComunAcciones.establecerProcesando({ procesando: false })
          );
          resolve(respuesta);
        });
    });
  }

  guardar(objeto: SujetoIdentificado): Promise<RespuestaObjeto<SujetoIdentificado>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService
        .guardar(objeto)
        .subscribe((respuesta: RespuestaObjeto<SujetoIdentificado>) => {
          if (respuesta.tipoRespuesta === 'Exito') {
            if (this.paginado) {
              this.buscar(
                this.filtro,
                this.paginado.pagina,
                this.paginado.registrosPorPagina
              );
            }
          }
          this.store.dispatch(ComunAcciones.establecerRespuesta({ respuesta }));
          this.store.dispatch(
            ComunAcciones.establecerProcesando({ procesando: false })
          );
          resolve(respuesta);
        });
    });
  }

  modificar(id: number, objeto: any): Promise<RespuestaObjeto<SujetoIdentificado>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService
        .modificar(id, objeto)
        .subscribe((respuesta: RespuestaObjeto<SujetoIdentificado>) => {
          if (respuesta.tipoRespuesta === 'Exito') {
            if (this.paginado) {
              this.buscar(
                this.filtro,
                this.paginado.pagina,
                this.paginado.registrosPorPagina
              );
            }
          }
          this.store.dispatch(
            ComunAcciones.establecerProcesando({ procesando: false })
          );
          this.store.dispatch(ComunAcciones.establecerRespuesta({ respuesta }));
          resolve(respuesta);
        });
    });
  }

  eliminar(id: number): Promise<Respuesta> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService.eliminar(id).subscribe((respuesta: Respuesta) => {
        if (respuesta.tipoRespuesta === 'Exito') {
          if (this.paginado) {
            this.buscar(
              this.filtro,
              this.paginado.pagina,
              this.paginado.registrosPorPagina
            );
          }
        }
        this.store.dispatch(ComunAcciones.establecerRespuesta({ respuesta }));
        this.store.dispatch(
          ComunAcciones.establecerProcesando({ procesando: false })
        );
        resolve(respuesta);
      });
    });
  }

  obtenerCodificador(): Promise<RespuestaLista<Codificador>> {
    this.store.dispatch(
      ComunAcciones.establecerProcesando({ procesando: true })
    );
    return new Promise((resolve) => {
      this.sujetoIdentificadoService
        .obtenerCodificador()
        .subscribe((respuesta: RespuestaLista<Codificador>) => {
          this.store.dispatch(
            ComunAcciones.establecerProcesando({ procesando: false })
          );
          resolve(respuesta);
        });
    });
  }
}
